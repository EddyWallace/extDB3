...
void MariaDBStatement::init(db::AbstractConnector& connector)
{
	connector_ptr = dynamic_cast<MariaDBConnector*>(&connector);
}
...
void MariaDBStatement::bindParams(std::vector<db::SQL_BIND_PARAMS>& params)
{
	...
	for (int i = 0; i < params_count; i++)
	{
		...
		MariaDBStatement::mysql_bind_param& param = reinterpret_cast<MariaDBStatement::mysql_bind_param&>(params[i]);
...
void MariaDBStatement::execute(std::vector<db::SQL_OPTION>& output_options, std::string& strip_chars, int& strip_chars_mode, std::string& insertID, std::vector<std::vector<std::string>>& results)
{
