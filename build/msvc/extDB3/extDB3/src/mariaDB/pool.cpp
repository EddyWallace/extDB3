...
std::unique_ptr<db::AbstractPool::SQL_SESSION> MariaDBPool::get()
{
	std::unique_ptr<db::AbstractPool::SQL_SESSION> mariadb_session;
	{
		...
		if (mariadb_session_pool.size() > 0)
		{
			...
			mariadb_session.reset(new db::AbstractPool::SQL_SESSION());
...
void MariaDBPool::putBack(std::unique_ptr<db::AbstractPool::SQL_SESSION> mariadb_session)
{
