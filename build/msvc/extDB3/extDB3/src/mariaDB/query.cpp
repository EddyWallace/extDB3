...
void MariaDBQuery::init(db::AbstractConnector& connector)
{
	connector_ptr = dynamic_cast<MariaDBConnector*>(&connector);
}


void MariaDBQuery::get(std::vector<db::SQL_OPTION>& output_options, std::string& strip_chars, int& strip_chars_mode, std::string& insertID, std::vector<std::vector<std::string>>& result_vec)
{
	result_vec.clear();
	do {
